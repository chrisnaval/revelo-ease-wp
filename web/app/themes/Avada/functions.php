...
/**
 * Function for calling 3rd party API, in demo for creating a user.
 */
add_action( 'gform_after_submission_2', 'post_to_third_party', 10, 2 );
function post_to_third_party( $entry, $form ) {
 
	// default domain = https://demo.api.easehub.online
	$post_url = 'https://demo.api.easehub.online/merchant/demo-user';
    $body = array(
        'first_name' => rgar( $entry, '1.3' ),
        'last_name' => rgar( $entry, '1.6' ),
        'email' => rgar( $entry, '2' ),
        'contact_no' => rgar( $entry, '3' ),
        );
    GFCommon::log_debug( 'gform_after_submission: body => ' . print_r( $body, true ) );
 
    $request = new WP_Http();
    $response = $request->get( $post_url, array( 'body' => $body ) );
    GFCommon::log_debug( 'gform_after_submission: response => ' . print_r( $response, true ) );
}

/**
 * Fix for tab index, because there are two forms in the same page
 * Add tab index for each form
 */
add_filter( 'gform_tabindex_1', function() { return 10; } );
add_filter( 'gform_tabindex_2', function() { return 32; } );

/**
 * Fix for form anchor in request for demo login form
 */
